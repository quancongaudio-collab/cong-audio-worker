...
def main():
    ...
    parser.add_argument("--drive-file-id", type=str, default=None,
                         help="Chỉ xử lý đúng 1 video này (bỏ qua bộ lọc transcript_segments IS NULL, "
                              "chạy lại được kể cả video đã xử lý rồi) — dùng để test nhanh 1 video cụ thể")
    ...
    if args.drive_file_id:
        files = [args.drive_file_id]
        print(f"Xử lý riêng 1 video: {args.drive_file_id}")
    else:
