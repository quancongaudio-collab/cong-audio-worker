...
def render_story_video(drive_service, data):
    ...
    data (dict) cần có các khoá:
      ...
      duration (float)             - thời lượng đoạn cắt (giây); mục tiêu 20-40s nhưng
                                      được linh hoạt +/-7s (13-47s) để không cắt cụt câu
                                      — xem cut_point.py, KHÔNG ép cứng về 20-40 nữa
    ...
    # Mục tiêu vẫn 20-40s, nhưng cho phép linh hoạt +/-7s (13-47s) để giữ trọn câu nói
    # — điểm start/end thật đã được chốt đúng ở bước index (cut_point.py), ở đây chỉ
    # chặn các giá trị bất thường (âm, quá lớn), không ép cứng về 20-40 như trước nữa.
    duration = min(max(float(data.get("duration") or 30), 13), 47)
