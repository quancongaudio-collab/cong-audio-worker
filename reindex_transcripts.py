...
CÁCH DÙNG:
  ...
  python3 reindex_transcripts.py --drive-file-id XXXX   # chỉ xử lý đúng 1 video
                                                          # (dùng để test nhanh 1 video cụ thể,
                                                          # chạy lại được kể cả đã xử lý rồi)
...
def main():
    ...
    parser.add_argument("--drive-file-id", type=str, default=None,
                         help="Chỉ xử lý đúng (các) video này, cách nhau bằng dấu phẩy nếu nhiều "
                              "(vd: --drive-file-id ID1,ID2,ID3). Bỏ qua bộ lọc transcript_segments "
                              "IS NULL, chạy lại được kể cả video đã xử lý rồi — dùng để test/xử lý "
                              "lại đúng những video cụ thể.")
    ...
    if args.drive_file_id:
        files = [f.strip() for f in args.drive_file_id.split(",") if f.strip()]
        print(f"Xử lý riêng {len(files)} video: {files}")
    else:
