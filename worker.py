...
# Kích thước file gốc tối đa (bytes) trước khi bỏ qua và chuyển thẳng vào
# needs_review. Các video vượt ngưỡng này (thường 4K/HEVC bitrate cực cao)
# có nguy cơ cao làm tràn /tmp (giới hạn cứng 2GB của Render, không tăng được
# dù nâng cấp gói) trong lúc vừa giữ file gốc vừa ghi file đã nén dở. Kiểm
# tra NGAY từ size_bytes có sẵn trong metadata Google Drive — tức là bỏ qua
# trước cả khi tải file về, không tốn băng thông/thời gian vô ích.
MAX_VIDEO_SIZE_BYTES  = int(1.2 * 1024 * 1024 * 1024)  # 1.2 GB
# ─────────────────────────────────────────────
...
def transcribe_audio(audio_path: str) -> dict:
    """
    Gọi Whisper qua Groq với response_format="verbose_json" để lấy kèm mốc giờ
    theo từng câu (segments), thay vì chỉ lấy văn bản thô như trước — đây là dữ
    liệu bắt buộc để chọn điểm cắt video không cụt câu (xem cut_point.py).

    Trả về dict: {"text": str, "segments": [{"start": float, "end": float, "text": str}, ...]}
    Nếu lỗi hoặc không có audio, trả về {"text": "", "segments": []}.
    """
    empty = {"text": "", "segments": []}
    if not GROQ_API_KEY:
        return empty
...
def save_video_records(conn, common_meta: dict, segments: list) -> list:
    ...
    for i, seg in enumerate(segments):
        ...
        rows.append((
            ...
            Json(common_meta.get("transcript_segments", [])),
    ...
    with conn.cursor() as cur:
        execute_values(cur, """
            INSERT INTO videos (
                ...
                target_audience, copyright_status, transcript, transcript_segments,
...
def process_video(drive_service, conn, video_info: dict) -> dict:
    ...
    if size_bytes and size_bytes > MAX_VIDEO_SIZE_BYTES:
        msg = (f"Video quá lớn ({size_bytes/1e9:.2f} GB > "
               ...
               f"cần xử lý thủ công (nén tay hoặc bỏ qua)")
        print(f"[SKIP-TOO-LARGE] {file_name}: {msg}")
        mark_error(conn, file_id, msg)
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        try:
            ...
            # Chốt lại start_time/end_time của từng đoạn theo mốc giờ Whisper thật,
            # áp 7 quy tắc chọn điểm cắt (xem cut_point.py). AI-Vision chỉ đoán gần
            # đúng dựa trên frame ảnh -> ở đây thay bằng điểm cắt chính xác không
            # cụt câu. Đoạn nào không tìm được điểm cắt hợp lệ trong toàn video sẽ
            # bị loại bỏ hẳn (theo đúng yêu cầu "cụt nghĩa thì vứt bỏ").
            refined_segments = []
            for seg in ai_data.get("valuable_segments", []):
                requested_start_sec = timestamp_to_seconds(seg.get("start_time"))
                window = select_cut_window(transcript_segments, requested_start_sec)
                if window is None:
                    print(f"        [BỎ QUA] Đoạn '{seg.get('summary', '')[:40]}...' "
                          f"không tìm được điểm cắt không cụt câu — loại bỏ.")
                    continue
                start_sec, end_sec = window
                seg = {**seg, "start_time": seconds_to_timestamp(start_sec),
                       "end_time": seconds_to_timestamp(end_sec)}
                refined_segments.append(seg)
            had_original_segments = bool(ai_data.get("valuable_segments"))
            if had_original_segments and not refined_segments:
                # Tất cả đoạn giá trị đều cụt câu -> KHÔNG fallback lưu nguyên cả
                # video (vì fallback đó cũng sẽ cụt nghĩa/dài bất thường), bỏ hẳn
                # theo đúng yêu cầu "cụt nghĩa thì vứt bỏ".
                print(f"        [BỎ QUA VIDEO] Tất cả {n_segs} đoạn giá trị đều bị loại vì "
                      f"không cắt được câu trọn vẹn — không lưu record nào cho video này.")
                mark_done(conn, file_id, None)
                return {"status": "skipped_no_valid_cut", "cost_usd": cost_usd}
            common_meta = {
            ...
            video_ids = save_video_records(conn, common_meta, refined_segments)
